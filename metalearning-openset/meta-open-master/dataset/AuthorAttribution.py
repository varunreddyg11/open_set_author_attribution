...
class AuthorAttribution(Dataset):
    """
...
"""
    ...
    def __init__(self, root, resize, split, mode, augment):
        ...
        df = pd.read_csv("meta-open-master/dataset/authorattribution/train_1.csv")
        df.dropna(inplace=True)
        classes = sorted(df["real_label"].dropna().drop_duplicates().to_list())
        class_to_idx = {j:i for i,j in enumerate(classes)}
        samples = []
        targets = []
        for idx,row in df.iterrows():
            samples.append((df.loc[idx,"review"],class_to_idx[df.loc[idx,"real_label"]]))
            targets.append(class_to_idx[df.loc[idx,"real_label"]])
