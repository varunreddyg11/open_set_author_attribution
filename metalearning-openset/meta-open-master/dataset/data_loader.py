...
from dataset.AuthorAttribution import AuthorAttribution
...
def data_loader(opts, opts_runtime, split):
    ...
    elif opts.data.name == 'authorattribution':
        relative_path = 'dataset/authorattribution/'
        print(_curr_str)
        data = AuthorAttribution(
            root=relative_path,
            resize=opts.data.im_size, split=split, mode=opts.train.mode, augment=augment)
        print('\t\tFind {:d} closed samples'.format(data.closed_samples))
        if data.open_samples > 0:
            print('\t\tFind {:d} open samples'.format(data.open_samples))
